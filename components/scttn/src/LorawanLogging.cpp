...
namespace sc::lorawan
{
    ...
    static void printMessage(LorawanLogMessage* log);
    static void printFatalError(LorawanLogMessage* log);
    static void printEvent(LorawanLogMessage* log);
    static void printEvtJoined(LorawanLogMessage* log);
    static void printEvtJoinFailed(LorawanLogMessage* log);
    static void printEvtTxComplete(LorawanLogMessage* log);
    static void printEvtTxStart(LorawanLogMessage* log);
    static void printEvtRxStart(LorawanLogMessage* log);
    static void printEvtJoinTxComplete(LorawanLogMessage* log);
    static void bin2hex(const uint8_t* bin, unsigned len, char* buf, char sep = 0);

    // Create singleton instance
    LorawanLogging* LorawanLogging::initInstance()
    {
        ttnLog.init();
        return &ttnLog;
    }
    ...
    // Initialize logging
    void LorawanLogging::init()
    {
        ringBuffer = xRingbufferCreate(NUM_RINGBUF_MSG * sizeof(LorawanLogMessage), RINGBUF_TYPE_NOSPLIT);
    ...
    void LorawanLogging::logEvent(int event, const char* message, uint32_t datum)
    {
        if (ringBuffer == nullptr)
            return;

        LorawanLogMessage log;
        log.message = message;
        log.datum = datum;
    ...
    void LorawanLogging::logFatal(const char* file, uint16_t line)
    {
    ...
    void LorawanLogging::loggingTask(void* param)
    {
        RingbufHandle_t ringBuffer = (RingbufHandle_t)param;

        while (true) {
            size_t size;
            LorawanLogMessage* log = (LorawanLogMessage*) xRingbufferReceive(ringBuffer, &size, portMAX_DELAY);
            if (log == nullptr)
                continue;
    ...
    void printMessage(LorawanLogMessage* log)
    {
    ...
    void printFatalError(LorawanLogMessage* log)
    {
    ...
    void printEvent(LorawanLogMessage* log)
    {
    ...
    void printEvtJoined(LorawanLogMessage* log)
    {
    ...
    void printEvtJoinFailed(LorawanLogMessage* log)
    {
        rps_t rps = log->rps;
    ...
    void printEvtTxComplete(LorawanLogMessage* log)
    {
        rps_t rps = log->rps;
    ...
    void printEvtTxStart(LorawanLogMessage* log)
    {
        rps_t rps = log->rps;
        ESP_LOGI(TAG, "- ch=%d, rps=0x%02x (%s, %s, %s, %s, IH=%d)",
            (unsigned)log->txChnl,
    ...
    void printEvtRxStart(LorawanLogMessage* log)
    {
        rps_t rps = log->rps;
    ...
    void printEvtJoinTxComplete(LorawanLogMessage* log)
    {
